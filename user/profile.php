<?php
// Profile page where the logged-in user can review and update account data.
...
if ($db === null) {
    ...
    // Load the current user first so the form starts with saved values.
    $userStatement = $db->prepare('SELECT name, email, phone, address, created_at FROM users WHERE id = ? AND role = ? LIMIT 1');
    $userRole = 'user';

    if ($userStatement === false) {
        $pageError = 'Unable to load your profile information.';
    } else {
        $userStatement->bind_param('is', $userId, $userRole);
        $userStatement->execute();
        $userStatement->store_result();

        if ($userStatement->num_rows !== 1) {
            $userStatement->close();
            set_flash_message('Unable to find your account.', 'error');
            redirect('index.php');
        }

        $userStatement->bind_result($savedName, $savedEmail, $savedPhone, $savedAddress, $savedCreatedAt);
        $userStatement->fetch();
        $userStatement->close();
        ...
        if (is_post_request()) {
            ...
            if (empty($errors)) {
                ...
                if ($updateStatement === false) {
                    ...
                    $updateStatement->bind_param('sssis', $name, $phone, $address, $userId, $userRole);
...
<main class="container">
    ...
    <section class="page-banner">
        <div class="dashboard-header">
            <div>
                <span class="section-tag">My Profile</span>
                <h1>My Profile</h1>
                <p>View your account details and update your contact information.</p>
            </div>
            <span class="status-pill">User Account</span>
        </div>
